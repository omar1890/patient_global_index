...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.vaccine.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vaccines.update", [$vaccine->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.vaccine.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $vaccine->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('start_date') ? 'is-invalid' : '' }}" type="text" name="start_date" id="start_date" value="{{ old('start_date', $vaccine->start_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text" name="end_date" id="end_date" value="{{ old('end_date', $vaccine->end_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('times_vaccinated') ? 'is-invalid' : '' }}" type="text" name="times_vaccinated" id="times_vaccinated" value="{{ old('times_vaccinated', $vaccine->times_vaccinated) }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_annual') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_annual" id="is_annual" value="1" {{ $vaccine->is_annual || old('is_annual', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('patient') ? 'is-invalid' : '' }}" name="patient_id" id="patient_id" required>
                    @foreach($patients as $id => $entry)
                        <option value="{{ $id }}" {{ (old('patient_id') ? old('patient_id') : $vaccine->patient->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
