    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.medicines.update", [$medicine->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $medicine->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.medicine.fields.type') }}</label>
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    <option value disabled {{ old('type', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Medicine::TYPE_SELECT as $key => $label)
                ...
                @endif
                <span class="help-block">{{ trans('cruds.medicine.fields.type_helper') }}</span>
            </div>
            <div class="form-group">
